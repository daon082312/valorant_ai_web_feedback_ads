...
class AsyncVideoDecoder:
    ...
    def open(self, path: str | Path) -> None:
        self.close()
        self._stop.clear()
        self._thread = threading.Thread(target=self._worker, args=(str(path),), daemon=True, name="ValorantVideoDecoder")
        self._thread.start()
    ...
    def seek(self, seconds: float, resume: bool = False) -> None:
        """Seek and optionally resume as one decoder-side operation."""
        kept: list[tuple] = []
        try:
            while True:
                item = self.commands.get_nowait()
                if not item or item[0] not in {"seek", "play", "pause"}:
                    kept.append(item)
        except queue.Empty:
            pass
        for item in kept:
            self.commands.put(item)
        self.commands.put(("seek", max(0.0, float(seconds)), bool(resume)))
    ...
    def close(self) -> None:
        ...
        thread = self._thread
        if thread is not None and thread.is_alive(): thread.join(timeout=0.8)
        self._thread = None
        self._clear_queue(self.messages); self._clear_queue(self.commands)
    ...
    def _publish(self, message: tuple) -> None:
        ...
        kind = message[0]
        if kind == "frame":
            try: self.messages.put_nowait(message)
            except queue.Full: pass
            return
        try:
            self.messages.put_nowait(message); return
        except queue.Full:
            pass
        kept: list[tuple] = []
        removed_frame = False
    ...
    def _worker(self, path: str) -> None:
        ...
        if not cap.isOpened():
            self._publish(("error", "선택한 영상을 열 수 없습니다.")); return
        ...
        read_and_publish(); current = 0.0

        while not self._stop.is_set():
            try:
                while True:
                    cmd = self.commands.get_nowait(); kind = cmd[0]
                    if kind == "stop":
                        self._stop.set(); break
                    if kind == "pause":
                        playing = False
                    elif kind == "play":
                        playing = True; next_deadline = time.monotonic()
                    elif kind == "seek":
                        resume = bool(cmd[2]) if len(cmd) > 2 else False
                        playing = False
                        target = max(0.0, min(float(cmd[1]), duration if duration > 0 else float(cmd[1])))
                        cap.set(cv2.CAP_PROP_POS_MSEC, target * 1000.0)
                        current = target
                        if read_and_publish(): self._publish(("seeked", current))
                        else: self._publish(("seeked", target))
                        playing = resume
                        if playing: next_deadline = time.monotonic()
            except queue.Empty:
                pass
            ...
            if now < next_deadline:
                time.sleep(min(0.010, next_deadline - now)); continue
            if not read_and_publish():
                playing = False; self._publish(("eof", current)); continue
