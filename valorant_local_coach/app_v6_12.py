...
class App(V611App):
    ...
    def _render_fight(self, fight) -> None:
        ...
        metrics = {"movement_score": fight.movement_score, "aim_score": aim.get("aim_score"), "skill_score": None if mode == "brawl" else fight.skill_timing_score, "moving_shot_ratio": fight.moving_shot_ratio, "stop_to_shot_ms": fight.median_stop_to_shot_ms, "avg_shots_per_fight": fight.shots}
    ...
    def _session_learning_metrics(self, summary: dict, vision: dict) -> dict:
        ...
        return {"movement_score": movement, "aim_score": vision.get("aim_score"), "skill_score": None if getattr(self, "_brawl_mode", False) else skill, "moving_shot_ratio": metrics.get("moving_shot_ratio"), "stop_to_shot_ms": metrics.get("median_stop_to_shot_ms"), "avg_shots_per_fight": avg_shots, "fight_count": summary.get("fight_count", 0), "shots": summary.get("shots", 0)}
    ...
    def stop_session(self) -> None:
        ...
        valid_for_learning = bool(self.config_data.get("personal_learning_enabled", True)) and int(metrics.get("fight_count") or 0) >= 1 and int(metrics.get("shots") or 0) >= 2
    ...
    def _preview_tick(self) -> None:
        try:
            frame = None
            ts = 0.0
            fps = max(1.0, min(12.0, float(self.config_data.get("preview_fps", 3))))
            with self._preview_lock:
                if self._preview_frame is not None and self._preview_timestamp > self._preview_rendered_timestamp and self._preview_timestamp - self._preview_rendered_timestamp >= 1.0 / fps:
                    frame = self._preview_frame.copy()
                    ts = self._preview_timestamp
            if frame is not None:
                if self.config_data.get("show_vision_boxes_in_preview", False):
                    annotated = self.vision.latest_annotated()
                    if annotated is not None:
                        frame = annotated
                self._show_frame(frame)
                self._preview_rendered_timestamp = ts
        finally:
    ...
    def _chat_context(self) -> dict:
        ...
        context["personal_learning"] = {"enabled": bool(self.config_data.get("personal_learning_enabled", True)), "mode": mode, "baseline": self.personal_profile.baseline(mode)}
