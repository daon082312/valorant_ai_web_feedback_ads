...
def _apply_aim_gate(candidate_idx: int, aim_score: float | None) -> int:
    if aim_score is None:
        return min(candidate_idx, 5)
    idx = int(candidate_idx)
    while idx >= 4:
        required = AIM_GATES.get(idx)
        if required is None or float(aim_score) >= required:
            break
        idx -= 1
    return max(0, idx)
...
def predict_tier(report: dict) -> dict:
    ...
    raw = {
        "aim": (aim_value, 0.38, max(0.28, aim_conf)),
        "headline": (_num(report.get("headline_score")), 0.14, max(0.25, aim_conf)),
        "movement": (_num(report.get("movement_score")), 0.23, min(1.0, 0.35 + bursts / 10.0)),
    ...
    if not available:
        return {
            "tier": "Unrated", "tier_ko": "판정 불가", "tier_color": "#98A2B3",
            "score": None, "confidence": 0.0, "confidence_label": "low", "basis": [],
            "aim_gate": None, "note": "single_video_mechanics_tier_v713_not_account_rank",
        }
    ...
    confidence = (
        0.30 * metric_evidence + 0.25 * burst_evidence
        + 0.20 * duration_evidence + 0.25 * reliability_mean
    )
    confidence = max(0.08, min(0.95, confidence))

    raw_idx = _tier_index(score)
    idx = _apply_aim_gate(raw_idx, aim_value)
    tier, tier_ko, tier_color = TIERS[idx]
    label = "high" if confidence >= 0.72 else "medium" if confidence >= 0.45 else "low"

    return {
        "tier": tier, "tier_ko": tier_ko, "tier_color": tier_color,
        "tier_index": idx, "raw_tier_index": raw_idx,
        "score": round(score, 1), "confidence": round(confidence, 3),
        "confidence_label": label, "basis": [x[0] for x in available],
        "raw_metrics": {x[0]: round(x[3], 1) for x in available},
        "thresholds": list(THRESHOLDS), "aim_gates": dict(AIM_GATES),
        "aim_gate": AIM_GATES.get(idx),
        "note": "single_video_mechanics_tier_v713_not_account_rank",
    }
